...
for i in range(len(geonames)):
    if "P" == geonames.loc[i,6]:
        entry = {"name": geonames.loc[i,2].lower(), "lat": geonames.loc[i,4], "lon":geonames.loc[i,5], "geonameid":geonames.loc[i,0], "featureclass":geonames.loc[i,6]}
...
f_out_city = open("pa_eq_city_match.csv","w+")
f_out_region = open("pa_eq_region_match.csv","w+")
...
for jot in range(len(jot_cities)):
    ...
    if found_city_match > 1:  
        for geo in range(len(geoname_region)):
            if jot_city == geoname_region[geo]["name"]:
